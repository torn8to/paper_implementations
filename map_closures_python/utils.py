...
def transform_cloud(cloud: np.ndarray, transform: np.ndarray) -> np.ndarray:
    """transforms_cloud relative to the transform frame"""
    padded_cloud = np.hstack([cloud, np.ones((cloud.shape[0], 1))])
    transformed_cloud_with_padding = (transform @ padded_cloud.T).T
    return transformed_cloud_with_padding[:, :3]
...
def plot_transformation_matrices(transforms, connections=None, figsize=(10, 8), title="Transformation Matrices in XY Plane"):
    ...
    for i, (x, y) in enumerate(zip(x_coords, y_coords)):
        ax.annotate(str(i), (x, y), xytext=(5, 5), textcoords="offset points", fontsize=8, alpha=0.8)
    ...
    if connections is not None:
        for idx, (i, j) in enumerate(connections):
            if i < len(transforms) and j < len(transforms):
                ...
                ax.plot([x1, x2], [y1, y2], color="orange", linewidth=2, alpha=0.7, label="Loop Closure" if idx == 0 else "")
...
def cloud_visualization(source_np: np.ndarray, target_np: np.ndarray):
