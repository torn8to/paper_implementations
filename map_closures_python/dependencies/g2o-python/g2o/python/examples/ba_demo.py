...
def main():
    ...
    num_pose = 15
    true_poses = [g2o.SE3Quat(np.identity(3), [i * 0.04 - 1, 0, 0]) for i in range(num_pose)]
    ...
    for i, point in enumerate(true_points):
        visible = [(j, z) for j, z in enumerate(cam.cam_map(pose * point) for pose in true_poses) if 0 <= z[0] < 640 and 0 <= z[1] < 480]
        if len(visible) < 2:
            continue
    ...
    sse[1] = sum(np.sum((optimizer.vertex(i).estimate() - true_points[inliers[i]]) ** 2) for i in inliers)
