...
def main():
  ...
  for i, point in enumerate(true_points):
    ...
    if inlier:
      ...
      error = true_poses[anchor].inverse() * invert_depth(v_p.estimate()) - true_points[i]
  ...
  for i in inliers:
    ...
    error = v_anchor.estimate().inverse() * invert_depth(v_p.estimate()) - true_points[inliers[i][0]]
