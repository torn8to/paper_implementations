...
class LoopClosureDetector:
    ...
    def add(self, bev_img, id: int):
        """Extract features and descriptors and add them hbst matching tree"""
        keypoints, descriptors = self.orb.detectAndCompute(bev_img.img, None)
        keypoints_list = [key.pt for key in keypoints]
        self.tree.add(keypoints_list, descriptors.tolist(), id, self.tree_split)

    def match(
        self, density_img: np.ndarray, position: np.ndarray, id: int
    ) -> Optional[
        tuple[
            int,
            RansacReturnModel,
        ...
        """Extract features, match against the database
        ...
        keypoints_list = [key.pt for key in keypoints]
        tree_matches = self.tree.match(keypoints_list, descriptors.tolist(), self.max_hamming_distance, self.tree_split)
        k = id
